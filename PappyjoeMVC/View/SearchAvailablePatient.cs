...
{
    public partial class SearchAvailablePatient : Form
    {
        ...
        public SearchAvailablePatient()
        {
            InitializeComponent();
        }

        public SearchAvailablePatient(DataTable dtb, string doctor_id)
        {
            InitializeComponent();
            this.dtb = dtb;
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                if (btnOk.Text == "OK")
                {
                   ...
                   DataTable dtb= mdl.Get_DoctorId(name);// cn.scalar("select id from tbl_doctor where doctor_name='" + name + "'");
                    drctId = dtb.Rows[0][0].ToString();
                    if (id != "")
                    {
                        ...
                        this.Close();
                        bool SetFlag = true;
                        var form1 = new AddNewPatients(SetFlag);
                    }
                }
            }
            catch (Exception ex)
