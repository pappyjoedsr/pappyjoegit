...
namespace PappyjoeMVC.Model
{
    public class Patients_Report_model
    {
        ...
        {
            DataTable dtb_procedure = db.table("SELECT  cast(A.date as date) as date,A.procedure_name,B.doctor_name,g.group_id FROM tbl_completed_procedures A INNER JOIN tbl_doctor B ON A.dr_id=B.id inner join tbl_patient P on p.id=A.pt_id inner join tbl_pt_group g on p.id=g.pt_id  WHERE g.group_id='" + cmb + "'and A.date between '" + d1 + "'and '" + d2 + "' and P.Profile_Status !='Cancelled' ");     //("SELECT  cast(A.date as date) as date,A.procedure_name,B.doctor_name,p.group_id FROM tbl_completed_procedures A INNER JOIN tbl_doctor B ON A.dr_id=B.id inner join tbl_patient P on p.id=A.pt_id WHERE group_id='" + cmb + "'and A.date between '" + d1 + "'and '" + d2 + "' and P.Profile_Status !='Cancelled' ");
            return dtb_procedure;
        }
        public DataTable dtb_invoice(string d1,string d2,string cmb)
        {
            DataTable dt1 = db.table("SELECT a.id, a.pt_id,a.pt_name,invoice_no,services,unit,discount_type,g.group_id,a.date,d.doctor_name,total_discount,((cast(cost as decimal(17,1))* cast(unit as decimal(17,1)))+(CAST(total_tax as decimal(17,1)))-(CAST(total_discount as decimal(17,1))) ) as Cost,cost as Total_Cost,(((cast(cost as decimal(17,1))* cast(unit as decimal(17,1)))+(CAST(total_tax as decimal(17,1)))-(CAST(total_discount as decimal(17,1))))-(CAST(total as decimal(17,1))) ) as Payment,cast(total as decimal(17,1)) as due FROM `tbl_invoices` a LEFT JOIN tbl_doctor d  on a.dr_id=d.id LEFT JOIN tbl_patient p on a.pt_id=p.id inner join tbl_pt_group g on p.id=g.pt_id WHERE a.date between '"+ d1+"' and '"+ d2+"' and p.Profile_Status='Active' and g.group_id='"+cmb+"' order by a.date desc");//("select tbl_invoices.pt_id,tbl_invoices.pt_name,invoice_no,services,unit,discount_type,tbl_invoices.date,tbl_doctor.doctor_name,total_discount,((cast(cost as decimal(17,1))* cast(unit as decimal(17,1)))+(CAST(total_tax as decimal(17,1)))-(CAST(total_discount as decimal(17,1))) ) as Cost,cost as Total_Cost,(((cast(cost as decimal(17,1))* cast(unit as decimal(17,1)))+(CAST(total_tax as decimal(17,1)))-(CAST(total_discount as decimal(17,1))))-(CAST(total as decimal(17,1))) ) as Payment,cast(total as decimal(17,1)) as due,g.group_id from tbl_invoices LEFT JOIN tbl_doctor on tbl_invoices.dr_id=tbl_doctor.id LEFT JOIN tbl_patient on tbl_invoices.pt_id=tbl_patient.id inner join tbl_pt_group g on p.id=g.pt_id where  tbl_patient.Profile_Status='Active' and tbl_invoices.date between '" + d1 + "' and '" + d2 + "'and g.group_id='" + cmb + "' order by date desc");
            return dt1;
        }
        public DataTable dtb_receipt(string d1,string d2,string cmb)
        {
            DataTable dtp = db.table("SELECT distinct  A.pt_name,receipt_no ,A.invoice_no ,procedure_name, A.total ,A.amount_paid ,CAST((A.total-amount_paid) AS DECIMAL(17,2))  as 'DUE AFTER PAYMENT',payment_date, A.mode_of_payment ,d.doctor_name,g.group_id FROM tbl_payment A Left join tbl_doctor d on A.dr_id=d.id inner join tbl_patient P on p.id=A.pt_id inner join tbl_pt_group g on p.id=g.pt_id  Where payment_date between '" + d1 + "' and '" + d2 + "' and g.group_id='" + cmb + "' and P.Profile_Status !='Cancelled'");    //"SELECT distinct  A.pt_name,receipt_no ,A.invoice_no ,procedure_name, A.total ,A.amount_paid ,CAST((A.total-amount_paid) AS DECIMAL(17,2))  as 'DUE AFTER PAYMENT',payment_date, A.mode_of_payment ,d.doctor_name,group_id FROM tbl_payment A Left join tbl_doctor d on A.dr_id=d.id inner join tbl_patient P on p.id=A.pt_id  Where payment_date between '" + d1 + "' and '" + d2 + "' and P.group_id='" + cmb + "' and P.Profile_Status !='Cancelled'");
            return dtp;
        }
