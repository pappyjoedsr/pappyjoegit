...
namespace PappyjoeMVC.Model
{
    public class Daily_Invoice_Report_model
    {
        ...
        {
            DataTable dt = c.table("select DISTINCT id,doctor_name from tbl_doctor where not login_type = 'staff' and activate_login='yes' order by doctor_name");
            return dt;
        }
        public DataTable practicedetails()
        {
            DataTable dt = c.table("select name,contact_no,street_address,email,website  from tbl_practice_details");
            return dt;
        }
        public DataTable getinvoice(string invdate)
        {
            DataTable dt = c.table("select tbl_invoices.pt_name,tbl_invoices.date,invoice_no,services,cost,unit,discount_type,discountin_rs,tax_inrs,tbl_doctor.doctor_name from tbl_invoices LEFT JOIN tbl_patient on tbl_invoices.pt_id=tbl_patient.id LEFT JOIN tbl_doctor on tbl_invoices.dr_id=tbl_doctor.id where tbl_patient.Profile_Status='Active' and tbl_invoices.date='" + Convert.ToDateTime(invdate).ToString("yyyy-MM-dd")+"' order by date desc");
            return dt;
        }
        ...
        {
            DataTable dt = c.table("SELECT distinct  A.pt_name,receipt_no ,A.invoice_no ,procedure_name, A.total ,A.amount_paid ,CAST((A.total-amount_paid) AS DECIMAL(17,2))  as 'DUE AFTER PAYMENT',payment_date, A.mode_of_payment ,d.doctor_name FROM tbl_payment A Left join tbl_doctor d on A.dr_id=d.id inner join tbl_patient P on p.id=A.pt_id Where payment_date between '" + rcdte + "' and '" + rcdte2 + "' and P.Profile_Status !='Cancelled' ");
            return dt;
        }
        ...
        {
             string e=c.scalar("Select id from tbl_doctor where doctor_name='" + drname + "' and login_type='doctor'");
             return e;
        }
