...
namespace PappyjoeMVC.Model
{
    public class Consultation_model
    {
        ...
        {
            DataTable pid = db.table("SELECT MAX(id) from tbl_patient");
            return pid;
        }
        ...
        {
            int i = db.execute("update tbl_patient_automaticid set patient_number='" + n + "'");
            return i;
        }
        public int med_hist(string id,string grmedical)
        {
            int med = db.execute("insert into tbl_pt_medhistory (pt_id,med_id) values('" + id + "','" + grmedical + "')");
            return med;
        }
        public DataTable name_medhistory()
        {
            DataTable dt = db.table("SELECT name FROM tbl_medhistory order by name");
            return dt;
        }
        ...
        {
            DataTable dt = db.table("select DISTINCT id,doctor_name from tbl_doctor  where login_type='doctor' or login_type='admin'");
            return dt;
        }
