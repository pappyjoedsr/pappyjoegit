...
namespace PappyjoeMVC.Model
{
    public class LabtrackingReport_model
    {
        ...
        {
            DataTable dt = db.table("SELECT distinct A.id as'JOB.NO',B.pt_name as 'PATIENT INFO',B.primary_mobile_number as 'PHONE NO',C.doctor_name as'DOCTOR' ,A.labname as'LAB',A.work_name as'WORK NAME',A.duedate as'DUE',A.trackingstatus as'STATUS' from tbl_lab_main A inner join tbl_patient B on A.pt_id=B.id inner join tbl_doctor C on A.dr_id=C.id WHERE A.type='Dental' and B.primary_mobile_number like '" + phn_no + "%'");
            return dt;
        }
        ...
        {
            DataTable dt = db.table("SELECT distinct A.id as'Work ID',A.work_name as'Work Name',A.type as 'Work Type',B.pt_name ,A.pt_id from tbl_lab_main A inner join tbl_patient B on A.pt_id=B.id inner join tbl_doctor C on A.dr_id=C.id WHERE A.trackingstatus is null and type='Dental'");
            return dt;
        }
    }
}
